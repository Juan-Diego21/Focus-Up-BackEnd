...
export class UserService {
  ...
  async createUser(userData: UserCreateInput): Promise<{ success: boolean; user?: User; error?: string }> {
    try {
      // Validaciones
  ...
  async updateUser(id: number, updateData: UserUpdateInput): Promise<{ success: boolean; user?: User; error?: string }> {
    try {
      ...
      if (sanitizedData.nombre_usuario) {
        sanitizedData.nombre_usuario = ValidationUtils.sanitizeText(sanitizedData.nombre_usuario);
      }
