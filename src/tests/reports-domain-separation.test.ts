...
// JWT de prueba proporcionado por el usuario
...
describe('Reports Domain Separation Integration Tests', () => {
  ...
  describe('GET /api/v1/reports/sessions', () => {
    it('debe retornar solo reportes de sesiones de concentración', async () => {
      ...
      if (response.body.data.length > 0) {
        ...
        expect(session).not.toHaveProperty('id_metodo'); // Solo como referencia en metodo_asociado
  ...
  describe('Error Handling', () => {
    it('debe manejar usuarios inexistentes', async () => {
      // Usar un JWT con userId inexistente (esto requeriría un JWT diferente)
      // Por ahora, solo validamos que los endpoints respondan correctamente
      const response = await request(app)
        .get('/api/v1/reports/sessions')
        .set('Authorization', `Bearer ${TEST_JWT}`)
